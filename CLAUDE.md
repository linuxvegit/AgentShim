...
**Canonical model (frozen-core):** providers read `CanonicalRequest::resolved_policy`, never raw inbound headers. `ReasoningEffort` is `Minimal|Low|Medium|High|Xhigh|Max` (declaration order = intensity; derives `Ord`). Outbound effort is **catalog-driven when available**: the pipeline copies the target model's discovered `reasoning_effort` list onto `ResolvedPolicy::supported_efforts`, and encoders clamp via `ReasoningEffort::clamp_to_advertised` (highest advertised tier ≤ requested, raw upstream spelling preserved) — so `claude-opus-4.8` gets `max`, `claude-opus-4.6` (no `xhigh`) steps an Xhigh request down to `high`, `gpt-5.5` (no `max`) clamps Max to `xhigh`. **Static fallback** when the model has no catalog metadata: `xhigh`/`max` → `"high"` for OpenAI Chat/Responses; Copilot Chat promotes to `"xhigh"` via `ProviderCapabilities::accepts_xhigh`. Anthropic emits `output_config.effort` + adaptive thinking under `anthropic-beta: effort-2025-11-24`.
