...
async fn anthropic_text_stream_sse_events() {
    ...
    assert!(text.contains("event: message_start"), "missing message_start\n{}", text);
    assert!(text.contains("event: content_block_start"), "missing content_block_start\n{}", text);
    assert!(text.contains("event: content_block_delta"), "missing content_block_delta\n{}", text);
    assert!(text.contains("event: content_block_stop"), "missing content_block_stop\n{}", text);
    assert!(text.contains("event: message_delta"), "missing message_delta\n{}", text);
    assert!(text.contains("event: message_stop"), "missing message_stop\n{}", text);
    ...
    // stop_reason
    assert!(text.contains("end_turn"), "missing stop_reason end_turn\n{}", text);
}
